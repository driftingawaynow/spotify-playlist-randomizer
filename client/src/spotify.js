...
export const generateRandomPlaylist = (randomGenre, randomYearRange, descYearRange) => axios.get('/me').then(res => {
    let userID = res.data.id;
    console.log(randomYearRange);
    console.log(userID);
        // create playlist
        axios.post(`/users/${userID}/playlists`, {
            name: `${descYearRange} ${randomGenre}`,
            public: false,
            description: `${descYearRange} ${randomGenre}`
        }).then(res => { 
            let playlistID = res.data.id; // get playlist ID
            //find tracks in genre
            console.log(randomGenre);
            if(randomGenre !== "CHAOS!") {
            axios.get(`/search?type=track&q=genre:${randomGenre}%20year:${randomYearRange}`).then(res => {
